...
| 5 | rule 4 — an ADR before any new dependency | Diffs `dependencies` (not `devDependencies`) against `main`. A new one requires a changed file under `docs/adr/` on the branch, **or** its package name already appearing in any file under `docs/adr/` (`B-19`: a Builder must not edit an ADR that already covers the package). |
