---
name: task-close
description: Close a task session — verify commits, tests, and dependencies against the CLAUDE.md rules, then write the session's ledger annotation. Use when the user says /task-close, "close this task", "wrap up the session", or when a task's work is finished and the ledger row needs its human columns filled in.
---

# task-close

Makes the second half of the `CLAUDE.md` session ritual executable. Checks the
three rules that a reviewer would otherwise have to check by hand, then writes
the four ledger cells no transcript can report.
...
Checks 2, 4 and 5 fail the run. Nothing is written to the ledger when any
