...
/**
 * The site nav bar (`AC-NAV-1`, `AC-NAV-2`): links to the GitHub repo, the
 * presentation and the technical walkthrough, plus one link that follows
 * auth status — `/tasks` once signed in, `/login` otherwise. Reads
 * `useAuth()` directly rather than taking a prop, so `/login` and the
 * protected layout mount it the same way. Rendered while `status` is
 * `"unknown"` (pre-mount) with that last link hidden, since neither
 * destination is correct yet.
 */
