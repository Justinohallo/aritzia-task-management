...
/**
 * The task list (`AC-LIST-1..4`, `AC-FILT-1..6`, `AC-DONE-1..3`,
 * `AC-DEL-1`, `AC-DEL-3..4`). Order is derived at render (`AC-LIST-3`),
 * never stored, so a restored record lands back in place. Three states
 * are told apart: not yet hydrated (a skeleton), no tasks (`AC-LIST-2`),
 * and tasks the active filter hides (`AC-FILT-5`), announced through the
 * one live region (`AC-FILT-6`).
 *
 * Delete goes through the optimistic mutation in `lib/tasks/mutations.ts`
 * (`AC-API-2`, `AC-API-9`); a leaving row takes the keyboard user's focus
 * with it, landing on the neighbouring row's matching control or the
 * active filter when none is left (`AC-A11Y-4`), moved only when focus was
 * inside the leaving row. `useSearchParams` needs a Suspense boundary
 * above it for static rendering, so this file supplies its own.
 */
...
import { useTaskDispatch, useTasks, useTasksHydrated } from "@/components/tasks/provider";
...
type RowControl = "completed" | "delete"; // the two focusable controls in a row, by their data-control name (task-item.tsx)

interface FocusIntent {
  leavingId: string; // the row that is leaving; spent once it is gone
  neighbourId: string | null; // the row to land on, or null when there is none
  control: RowControl;
}

// The row to land on when `index` leaves `visible`: the next one, else the previous, so
// repeated Delete walks down the list and the last row hands back to the one above it.
...
function TaskListBody() {
  ...
  const [deleting, setDeleting] = useState<readonly Task[]>([]); // in flight, in the order they were started (AC-API-11)
  ...
  // Record where focus should go once `task` leaves the view; the effect below moves it after the removal renders.
  const planFocus = useCallback(
    (task: Task, control: RowControl) => {
      ...
      if (!row || !row.contains(document.activeElement)) return; // pointer user's focus is left alone
  ...
  useEffect(() => {
    ...
    if (root.querySelector(`[data-task-id="${intent.leavingId}"]`)) return; // still on screen; intent stands
    ...
    const target = // a neighbour's delete may be disabled mid-create (AC-API-11); its checkbox never is
  ...
  const onCompletedChange = useCallback(
    (task: Task, completed: boolean) => {
      // A screen-reader user needs to hear a vanished control (`AC-FILT-6`) and have focus follow it (`AC-A11Y-4`).
