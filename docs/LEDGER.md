...
| 2026-09-03 | 753c4e94-3c82-569f-9a75-a635f69b3787 | T-18 | AC-NAV-1,AC-NAV-2,AC-UI-1,AC-UI-3 | 8.4 | 4.4 | 0.52 | 106 | 17,559 | 134,817 | 6,489,208 | 2.01 | claude-sonnet-5 100% | - | 0 | pass | Operator asked for a GitHub repo link in the nav. No AC of its own, so the commit cites AC-NAV-1/2 (nav renders on both routes) and AC-UI-1/3, reconfirmed with scripts/responsive-check.mjs (18/18 checks, /tasks controls >=44px moved 14->15). AC-UI-3 stays 'part' on the same jsdom-can't-lay-out basis as the rest of AC-UI (ADR-0006) -- the manual responsive-check run is the proof, not a new Jest test. Extended the existing site-nav and a11y tab-order tests rather than adding new ones (tests_added 0): test/navigation/site-nav.test.tsx asserts the new GitHub link's href on both routes, test/a11y.test.tsx's AC-A11Y-4 walk adds the new first tab stop. Noted the further tab-order drift on the already-open B-31 (stale AC-A11Y-4 dated record) rather than opening a new blocker. Interventions declined: single-shot session answering one direct request, no proposal/accept/reject cycle with the operator. Ledger row for this session (753c4e94) was created manually via 'ledger.py --transcript' before annotating, because the prior latest row was an unrelated earlier T-18 session and close.py's latest-row guard matches on task_id, not session_id -- since T-18 is a standing task reused across sessions, that guard would have silently overwritten that session's row; see B-32. |
