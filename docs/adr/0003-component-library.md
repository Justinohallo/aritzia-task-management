...
**Status:** Accepted, amended 2026-09-02 (ARCH-07) · **Date:** 2026-09-01 · **Criteria:** `AC-UI-5..6`, `AC-A11Y-1..6`
...
them to exist. The criterion is reworded (`B-26`) and the five files are
deleted (T-20), with `tw-animate-css`, whose only consumer was `dialog`.
The seam is unchanged: `components/ui/` still holds only app-agnostic
primitives, now enforced by a lint rule rather than a Jest test (T-19).
