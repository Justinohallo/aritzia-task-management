/**
 * `localStorage` adapter (ADR-0002; `AC-STATE-3..6`, `AC-AUTH-10`), the one
 * place the application touches it. Everything read back is untrusted
 * input and goes through `persistedEnvelopeSchema`; on any failure the
 * result is an empty list, never a throw or a partial list (`AC-STATE-5`).
 * Nothing here runs at module load, so importing this file on the server
 * is safe (`AC-STATE-6`); the caller decides when to read.
 */
...
/** Parse a raw stored string, testable without a DOM; `[]` unless it is a complete, valid, current-version envelope. */
...
/** Write the task list as a versioned envelope; a full or disabled storage is reported, not thrown, so the in-memory list survives it. */
